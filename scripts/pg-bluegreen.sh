...
cnpg_slot_sync_enabled() {
  local value=${1:-}
  [[ -z "${value}" || "${value}" == "true" ]]
}

missing_app_secret_keys() {
  local present_keys=$1
  local missing=()
...
restore_app_replicas() {
  ...
  for deploy in ${APP_DEPLOYMENTS}; do
    ...
    if [[ -z "${r}" || "${r}" == "null" ]]; then
      warn "no saved replica count for ${deploy}; restoring to 1"
      r=1
    fi
...
cmd_preflight() {
  ...
  local slot_sync
  if ! slot_sync=$(kctl get cluster "${BLUE_CLUSTER}" -o json 2>/dev/null \
      | jq -r '.spec.replicationSlots.synchronizeReplicas.enabled // "true"'); then
    err "unable to read replicationSlots.synchronizeReplicas.enabled from blue cluster"
    fail=1
    slot_sync=false
  fi
  if ! cnpg_slot_sync_enabled "${slot_sync}"; then
  ...
  if [[ -z "${present_keys}" ]]; then
    ...
    local missing_keys
    missing_keys=$(missing_app_secret_keys "${present_keys}")
    if [[ -n "${missing_keys}" ]]; then
      err "secret ${BLUE_APP_SECRET} missing key(s) '${missing_keys}' (have: ${present_keys})"
      fail=1
    fi
  fi
...
cmd_create_green() {
  ...
  local missing_keys
  missing_keys=$(missing_app_secret_keys "${keys}")
  [[ -z "${missing_keys}" ]] \
    || die "${GREEN_APP_SECRET} missing required key(s) '${missing_keys}' (have: ${keys})"
...
if [[ "${PG_BLUEGREEN_SOURCE_ONLY:-false}" != "true" ]]; then
  main "$@"
fi
